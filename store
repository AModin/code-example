...
class Tinder {
    ...
    @action getAddresses = async () => {
      try {
        ...
        const { data } = await axios(
          `https://geocode-maps.yandex.ru/1.x/?apikey=67a168c7-7fb7-4f78-ac7f-46a1acd709ad&lang=ru_RU&format=json&geocode=${searchString}`,
          {
            crossDomain: true,
          },
        );
    ...
    @action makeDecision = async decision => {
      try {
        ...
        if (data?.accepted === true) {
          ...
          dataLayer.push({
            event: 'event_b2b_pro_beetogether_deal_success',
            eventCategory: 'pro_beetogether',
            eventAction: 'deal_success',
          });
        }
    ...
    @action addAction = async () => {
      try {
        ...
        const dataLayer = window.dataLayer || [];
        dataLayer.push({
          event: 'event_b2b_pro_beetogether_promo_success',
          eventCategory: 'pro_beetogether',
          eventAction: 'first_promo_success',
          sequence: 'extra',
        });
      } catch (e) {
        console.log(e);
    ...
    @action addBusiness = async () => {
      ...
      try {
        ...
        const dataLayer = window.dataLayer || [];
        dataLayer.push({
          event: 'event_b2b_pro_beetogether_business_success',
          eventCategory: 'pro_beetogether',
          eventAction: 'business_success',
          sequence: 'extra',
        });
      } catch (e) {
        console.log(e);
    ...
    @action startWork = async () => {
      try {
        ...
        const dataLayer = window.dataLayer || [];
        dataLayer.push({
          event: 'event_b2b_pro_beetogether_reg_success',
          eventCategory: 'pro_beetogether',
          eventAction: 'reg_success',
        });
      } catch (e) {
        console.log(e);
      }
    ...
    @action goToSecondStep = async () => {
      ...
      dataLayer.push({
        event: 'event_b2b_pro_beetogether_business_success',
        eventCategory: 'pro_beetogether',
        eventAction: 'first_business_success',
        sequence: 'first',
      });
